...
const sendToAi = require('../../utils/openai/sendToAi_vola');
...
async function sendMessageV2(req, res) {
  ...
  try {
    ...
    const {
      ...
      status,
      error
    } = await sendToAi(messagesResult.recordset[0].message, message);
    if (!status) {
    ...
    const messagesFinalResult = await selectMessage({
      conversationId
    });
    console.log('flag 4');
    res.status(200).send({
