...
class MessageGenerator {
	...
	systemMessage(message) {
		return {
			role: 'system',
			content: `당신은 챗봇으로써 다음 내용을 읽고 유저의 질문에 대답해주세요. 내용은 전체 내용에서 질문과 연관된 부분울 추출한 것입니다. 답변에 사용하는 언어는 사용자가 사용하는 언어와 동일하게 해주세요. 만일 지문이 주어지지 않았다면 연관된 지문이 없다는 것이니, 연관된 지문을 찾지 못했다고 해주세요.
			content : ${message}`,
		};
	}
	userMessage(message) {
